...
namespace TSL
{
  ...
  public:
    ...
    const Matrix<_Type>& matrix0() const;
    ...
    virtual void get_jacobian_of_matrix0_mult_vector( const Vector<_Type> &state, 
  ...
  protected:
    ...
    /// \param m The matrix.
    virtual void matrix0( const Vector<_Type> &x, Matrix<_Type> &m ) const
    {
      std::string problem;
  ...
  private:
    ...
    Matrix<_Type> MATRIX0_AT_LAST_STATE;

  }
  ; // end class

  template <typename _Type, typename _Xtype>
  inline const Matrix<_Type>& Equation_1matrix<_Type, _Xtype>::matrix0() const
  {
